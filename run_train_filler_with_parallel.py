...
def rank_teacher_cache_path(cache_path: str, rank: int) -> str:
    """Return the temporary lightweight teacher-cache index for one rank.

    [Parallel teacher-cache change]
    大 logits 不保存在这个文件里，而是由 GenerationCache.add() 写入共享的 ``<cache_path>.shards/``。
    
    这个 rank 文件只记录本 rank 已完成样本的 prompt -> shard 文件映射，
    因此体积很小，也避免多个进程同时 torch.save() 同一个全局索引造成文件损坏。
    """
    return f"{cache_path}.rank{rank}.pt"


def merge_generation_cache(
    target: GenerationCache,
    source: GenerationCache,
) -> int:
    """Merge one rank's lightweight cache metadata into the global cache.

    [Parallel teacher-cache change]
    这里只合并 sequences/text 元数据和 cache_files 映射，不会把 shard 中的 teacher logits 读回 CPU，因此合并 10k 样本时也不会造成 CPU OOM。

    target: 全局缓存索引
    source: 某个 rank 的缓存索引

    """
    
    added = 0 # 记录有多少条 sample 是第一次加入 target
    # source.cache 是 dict[str, GenerationOutput]
    # content 为字典 key，也就是某条训练样本完整格式化后的 prompt 字符串
    # generation 包含 sequence、logits（空）、text
    for content, generation in source.cache.items():
        if content not in target.cache:
            added += 1
        target.cache[content] = generation
    # 把 source.cache_files 中全部键值对写入 target.cache_files
    target.cache_files.update(source.cache_files)
    return added


def save_generation_cache(cache: GenerationCache, cache_path: str) -> None:
    """Atomically centralize cache-index saving in one helper.
    把 GenerationCache 的轻量索引保存为一个 .pt 文件

    generation_cache：需要保存的缓存索引对象。
    cache_path：索引文件保存路径。

    [Parallel teacher-cache change]
    rank-local 索引和最终合并索引都使用相同格式：
    GenerationCache.to_state_dict()。真正的大 tensor 已在 shards 中。
    """
    ...
    # 把 GenerationCache 对象转换为普通字典，再保存为 pt 文件
    torch.save(cache.to_state_dict(), cache_path)
...
def train_one_config(
    ...
    # [Parallel teacher-cache change]
    # 并行预埋只在以下条件同时满足时启用：
    # 1. JSON 显式设置 parallel_teacher_cache=true；
    # 2. 当前由 torchrun/DDP 启动；
    # 3. cache_device="disk"，即 logits 会立即写入独立 shard。
    #
    # CPU/GPU resident cache 不适合并行预埋，因为每个 rank 都会在内存中
    # 保留自己的巨大 logits，并且无法通过一个轻量索引安全合并。
    parallel_teacher_cache = (
        train_config["parallel_teacher_cache"]
        and distributed
        and cache_offload_dir is not None
    )
    if (
        train_config["parallel_teacher_cache"]
        and not parallel_teacher_cache
        and main_process
    ):
        print(
            "Warning: parallel_teacher_cache only takes effect with "
            "DDP and cache_device='disk'. Falling back to rank0-only precompute."
        )
    ...
    if main_process:
        ...
        # [Parallel teacher-cache change]
        # 如果上一次并行预埋被中断，*.rankN.pt 可能已记录部分完成样本。
        # rank0 先把这些轻量索引并入全局索引，使本次启动跳过已完成 shard，而不需要从头重算。
        if parallel_teacher_cache:
            assert cache_path is not None
            merged_existing = 0

            # 遍历所有 rank
            for cache_rank in range(world_size):
                # 生成 rank-local 索引路径，每个文件只记录对应 rank 完成的样本索引
                rank_cache_file = rank_teacher_cache_path(
                    cache_path,
                    cache_rank,
                )
                # 检查临时索引是否存在
                ...
                # 加载 rank-local 索引，从 .rankN.pt 恢复一个 GenerationCache
                rank_cache = GenerationCache.load_from_file(
                    rank_cache_file,
                    device=cache_device,
                ) # cache_device 为 cpu，轻量索引加载到 cpu
                # 合并到全局索引，返回实际新增的样本数
                merged_existing += merge_generation_cache(
                    generation_cache,
                    rank_cache,
                )

            # 保存恢复后的全局索引
            # 非 rank0 随后需要读取这个文件来计算完全一致的 missing list
            save_generation_cache(generation_cache, cache_path)
            if merged_existing > 0:
                print(
                    f"Merged {merged_existing} samples from existing "
                    f"rank teacher-cache indexes into {cache_path}."
                )
    ...
    if parallel_teacher_cache:
        ...
        # [Parallel teacher-cache change]
        # 等待 rank0 完成旧索引加载/恢复。否则非 rank0 可能在全局索引尚未
        # 写好时读取，得到不同的 missing list，导致重复生成或漏生成。
        distributed_barrier(distributed, local_rank)

        if not main_process:
            generation_cache = GenerationCache.load_from_file(
                cache_path,
                device=cache_device,
            )
            cache_load_seconds = perf_counter() - cache_load_start
        ...
        # 找出缺失样本
        # 把训练样本转换成完整 prompt 字符串后检查 prompt 是否出此案在恢复后的全局索引中
        missing_indices = [
            sample_idx
        ...
        # 交错切分缺失样本，分给当前 rank
        # 例如 world_size=2：
        # rank0 取得 missing_indices[0], [2], [4]...
        # rank1 取得 missing_indices[1], [3], [5]...
        # 每条样本只属于一个 rank，因此基于 prompt hash 的 shard 不会竞争写入。
        rank_missing_indices = missing_indices[rank::world_size]
        # 取出当前 rank 的样本对象
        rank_teacher_samples = [
            samples[sample_idx]
            for sample_idx in rank_missing_indices
        ]

        if main_process:
            print(
                f"Teacher targets: {old_cache_len} cached, "
                f"{len(missing_indices)} to generate in parallel across "
                f"{world_size} ranks."
            )
        print(
            f"[rank{rank}] Teacher targets assigned: "
            f"{len(rank_teacher_samples)}/{len(missing_indices)} "
            f"missing samples.",
            flush=True,
        )

        # [Parallel teacher-cache change]
        # 每个 rank 使用独立 GenerationCache 元数据对象，但 offload_dir 相同。
        # add() 会把大 logits 直接保存到共享 shards 目录；rank-local .pt 只保存
        # 本 rank 的轻量映射，避免多个进程并发覆盖最终 cache_path。

        # 创建当前 rank 的独立轻量缓存
        rank_cache = GenerationCache(
            device=cache_device,
            offload_dir=cache_offload_dir,
        )
        # 生成当前 rank 的索引文件名
        rank_cache_file = rank_teacher_cache_path(cache_path, rank)

        # 当前 rank 生成 teacher logits
        rank_cache = build_generation_cache(
            rank_teacher_samples,  # 当前 rank 的样本
            gen_batch_size,  
            model,
            tokenizer,
            generation_config,
            rank_cache,
            store_logits=use_logits,
            cache_path=rank_cache_file, # 保存当前 rank 索引
            save_interval=generation_cache_save_interval, # 每隔多少 generation batch 保存一次
            log_prefix=f"[rank{rank}] ", # 日志表明来自哪个 rank
        )

        # 周期保存可能没有覆盖最后一个 batch，因此结束时必须再保存一次
        save_generation_cache(rank_cache, rank_cache_file)
        print(
            f"[rank{rank}] Saved teacher-cache index to "
            f"{rank_cache_file}, size={len(rank_cache.cache)}.",
            flush=True,
        )
        ...
        # 所有 rank 的 shard 和 rank-local 索引全部写完后，rank0 才能合并
        if main_process:
            for cache_rank in range(world_size):
                rank_cache_file = rank_teacher_cache_path(
                    cache_path,
                ...
                # 检查 rank 文件完整性，barrier 结束之后，理论上所有 rank 文件都应该存在
                # 如果确实，说明某个 rank 没有正确保存，不应该继续生成不完整的全局索引
                if not os.path.exists(rank_cache_file):
                    raise FileNotFoundError(
                        "Parallel teacher precompute finished without expected "
                        f"rank index: {rank_cache_file}"
                    )
                ...
                # 合并到全局缓存
                merge_generation_cache(generation_cache, rank_cache)

            # 统计合并后的全局缓存条目数量，并保存到 cache.disk.pt
            new_cache_len = len(generation_cache.cache)
            save_generation_cache(generation_cache, cache_path)
            print(
                f"Saved merged generation cache to {cache_path}, "
                f"size: {new_cache_len}"
            )
            ...
            print_duration(
                f"Teacher logits precompute "
                f"({new_cache_len - old_cache_len} samples)",
                teacher_seconds,
            )
        # 非 rank 0 不需要汇总，计时为 0
        else:
            teacher_seconds = 0.0

        # 等待 rank0 写好最终全局索引，非 rank0 等待 rank0 保存完成
        distributed_barrier(distributed, local_rank)
        
        # 从这里开始，所有 rank 都持有相同的完整轻量索引
        # 后续 DDP 训练时，每个 rank 可以按照自己的样本从共享 shards 中按需加载 teacher logits
        generation_cache = GenerationCache.load_from_file(
            cache_path,
            device=cache_device,
        )

        # 完整性检查：对于当前训练 samples，最终索引不应缺失任何 key。
        missing_after_merge = sum(
            sample_to_str(sample) not in generation_cache
            for sample in samples
        )
        if missing_after_merge:
            raise RuntimeError(
                "Merged teacher cache is incomplete: "
                f"{missing_after_merge}/{len(samples)} current samples missing."
            )

    elif main_process:
        # 原始兼容路径：未开启并行预埋时，仍然仅由 rank0 生成。
        old_cache_len = len(generation_cache.cache)
        ...
        generation_cache = build_generation_cache(
            ...
            log_prefix="[rank0] " if distributed else "",
        )
        new_cache_len = len(generation_cache.cache)
    ...
    if not main_process:
        # teacher logits 预埋结束之后，确保所有非 rank0 进程都拥有完整的全局索引，再进入 DDP
        if not parallel_teacher_cache:
            generation_cache = GenerationCache.load_from_file(
                cache_path,
                device=cache_device,
            )
